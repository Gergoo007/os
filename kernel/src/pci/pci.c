...
void pci_add_device(pci_hdr* d, u8 cfg_space, u8 bus, u8 dev, u8 fun) {
	...
	ddev.hdr.pci_addr.cfg_space = cfg_space;
	ddev.hdr.pci_addr.bus = bus;
	ddev.hdr.pci_addr.dev = dev;
	ddev.hdr.pci_addr.fun = fun;
	pci_hdr* hdr = kmalloc(sizeof(pci_hdr));
	memcpy(hdr, d, sizeof(pci_hdr));
	ddev.hdr.handle = hdr;

	switch (d->combclass) {
		case PCI_SATA: {
			ddev.hdr.type = DEV_PCI_AHCI;
			// ahci_init(d);
			break;
		}
		...
		case PCI_USB: {
			switch (d->prog_if) {
				case 0x00:
					ddev.hdr.type = DEV_UHCI;
					break;
				case 0x10:
					ddev.hdr.type = DEV_OHCI;
					break;
				case 0x20:
					ddev.hdr.type = DEV_EHCI;
					break;
				case 0x30:
					ddev.hdr.type = DEV_XHCI;
					break;
				case 0x80:
	...
	if (mc) {
		...
		foreach (i, num_config_spaces) {
			for (u32 bus = mc->config_spaces[i].bus_start; bus < mc->config_spaces[i].bus_end; bus++) {
				dev_pcibus* busdev = kmalloc(sizeof(dev_pcibus));
				...
				foreach (device, 32) {
					...
					foreach (f, 8) {
						volatile pci_hdr* func = (pci_hdr*) (dev_addr + (f << 12));
						if (func->vendor == 0 || func->vendor == 0xffff) continue;

						dev_misc* d = kmalloc(sizeof(dev_misc));
						memset(d, 0, sizeof(dev_misc));
						volatile dev_misc_pci_data* data = kmalloc(sizeof(dev_misc_pci_data));
						for (u32 k = 0; k < 6; k++)
